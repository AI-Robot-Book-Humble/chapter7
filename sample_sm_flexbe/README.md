# FlexBE States and Behaviors for sample_sm
...
This raw repository has several folders and files with the generic name `sample_sm`.
...
and change the relevant `sample_sm` text to `my_new_project_name` as needed.
...
[`copy_behavior`](sample_sm_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run sample_sm_flexbe_behavior copy_behavior` to see the usage guide. 
