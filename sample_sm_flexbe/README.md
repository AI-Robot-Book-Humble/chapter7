# FlexBE States and Behaviors for sample_sm_flexbe
...
This raw repository has several folders and files with the generic name `sample_sm_flexbe`.
...
and change the relevant `sample_sm_flexbe` text to `my_new_project_name` as needed.
...
## Example States in `sample_sm_flexbe_flexbe_states`
...
## Example Behaviors in `sample_sm_flexbe_flexbe_behaviors`
...
[`copy_behavior`](sample_sm_flexbe_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run sample_sm_flexbe_flexbe_behavior copy_behavior` to see the usage guide. 
